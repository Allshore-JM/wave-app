...
def _stations_with_live_spectral_wave_data() -> set:
    """
    Scans the realtime2 directory and returns station IDs that currently have
    raw spectral-density files.
    """
    html = _fetch_text(NDBC_REALTIME_DIR)
    return set(re.findall(r'href="([A-Za-z0-9]+)\.data_spec"', html))
...
def get_live_ndbc_wave_stations() -> list:
    """
    Join NDBC active station metadata to realtime spectral data availability.
    """
    if NDBC_STATIONS_CACHE["data"] and _cache_valid(NDBC_STATIONS_CACHE["timestamp"]):
        return NDBC_STATIONS_CACHE["data"]

    active = _parse_active_ndbc_stations()
    live_wave_ids = _stations_with_live_spectral_wave_data()

    stations = []

    for sid in sorted(live_wave_ids):
        info = active.get(sid)
        if not info:
            continue
...
def _parse_ndbc_spectral_file(text: str) -> list:
    """
    Parses NDBC realtime spectral files using rows like:

      YYYY MM DD hh mm Sep_Freq value (freq) value (freq) ...

    Returns rows with:
      timestamp_utc, freqs, values
    """
    rows = []
...
def _latest_spectral_row(rows: list) -> dict | None:
    ...
    return max(rows, key=lambda r: r["timestamp_utc"])
...
def _circular_mean_deg(degrees: list, weights: list) -> float | None:
    ...
    for deg, w in zip(degrees, weights):
        ...
        if not math.isfinite(deg) or not math.isfinite(w):
            continue
...
def _partition_spectrum(freqs: list, density: list, directions: list | None = None) -> list:
    """
    Splits the frequency spectrum into multiple swell/wind-sea components.
    """
    paired = []
    ...
    for i, peak_idx in enumerate(peaks):
        ...
        # Ignore very small noise partitions.
        if hs_ft < 0.25:
            continue
...
def api_ndbc_station_components(station_id):
    ...
    try:
        ...
        return jsonify({
            "station": station_id,
            "error": f"NDBC spectral-density file not available: {exc}"
        }), 404
    ...
    if direction_row:
        direction_by_freq = {
            round(f, 5): v
            for f, v in zip(direction_row["freqs"], direction_row["values"])
        }

        directions = [
            direction_by_freq.get(round(f, 5))
            for f in freqs
        ]
    ...
    NDBC_COMPONENT_CACHE[station_id] = {
        "timestamp": time.time(),
        "data": result
    }
